...
5. UI click-paths (drive with the /verify skill's Playwright setup; from the Task 8 review):
   - Legacy baseline: server without deploy_user shows the `/var/www/shipyard/` prefix in New Application, daemon/task user fields pre-fill www-data, no mismatch badges anywhere.
   - Provision empty server: Users panel create dialog pre-fills shipyard with the flag ON and no rationale paragraph; after create (no reload) the row shows the deploy user badge, the create dialog reopens with correct defaults (pins the onServerChange merge), New Application prefix reads `/home/shipyard/`, and daemon/task forms pre-fill shipyard on both server-scoped and app-scoped routes.
   - Adoption guard: server with legacy apps shows the flag OFF plus the rationale text; forcing it ON surfaces the 422 toast with no user created; "Set as deploy user" on a home-based account shows the confirm dialog, an in-flight spinner, and the 422 refusal.
   - Import warnings: provisioned server with misplaced apps shows one aggregated warning naming the paths, on first import AND re-import; a non-app directory produces a skipped entry but no warning.
   - Mismatch badges: pre-existing www-data daemon and scheduled task both show the amber badge in the header row with the directory still legible at ~1280px.
